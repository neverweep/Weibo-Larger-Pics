...
// @description    Easily view larger pics on Weibo.com 为图片框增加进入大图页面、图片详情页面和原始地址三个按钮
// @version        1.0.8
// @author         xiaoxia
...
// @updateinfo     考虑到未来版面可能有变化，将每种页面独立出来方便修改。
// ==/UserScript==
...
var uid, pid, mid, format, para, cdn, multiPics, quote;

//判断页面类型
if(document.getElementById('pl_content_homeFeed') != null){
    //本人时间线
    var nodeListen = document.getElementsByClassName('W_main')[0];
    nodeListen.addEventListener('DOMSubtreeModified',function(e){entryMain(e)});
}else if(document.getElementById('pl_content_hisFeed') != null && !enterprise){
    //他人时间线
    var nodeListen = document.getElementsByClassName('W_main')[0];
    nodeListen.addEventListener('DOMSubtreeModified',function(e){entryMain(e)});
}else if(enterprise){
     //企业版、政府版、专业版时间线
    var nodeListen = document.getElementsByClassName('W_main')[0];
    nodeListen.addEventListener('DOMSubtreeModified',function(e){entryEnterprise(e)});
}else if(document.getElementById('pl_weibo_feedlist') != null && search){
    //搜索页面
    var nodeListen = document.getElementById('pl_weibo_feedlist');
    nodeListen.addEventListener('DOMSubtreeModified',function(e){entrySearch(e)});
    ...
    nodeListen.addEventListener('DOMSubtreeModified',function(e){entryHot(e)});
}else if(document.getElementById('epfeedlist') != null && media){
    //媒体版页面
    var nodeListen = document.getElementById('epfeedlist');
    nodeListen.addEventListener('DOMSubtreeModified',function(e){entryMedia(e)});
    ...
    nodeListen_1.addEventListener('DOMSubtreeModified',function(e){entryHuati(e)});
    nodeListen_2.addEventListener('DOMSubtreeModified',function(e){entryHuati(e)});
    if(document.getElementById('pl_content_topicCite') != null){
        ...
        nodeListen.addEventListener('DOMSubtreeModified',function(e){entryHuati(e)});
    }
...
try{
    ...
    if(typeof(that.children) != 'undefined' && that.children.length > 0){
        ...
        //火狐修正
        if(isFirefox){
            var browserTest  = (that.className.match('WB_media_expand') != null && (that.className.match('SW_fun2') != null || that.className.match('SW_fun') != null) && that.style.display != "none") || (that.className == 'expand' && (that.parentNode.style.display != "none" && that.style.display != "none")) || (that.getAttribute('node-type') == 'imagesBox' && that.style.display != "none");
        }else{
            var browserTest = (that.className.match('WB_media_expand') != null && (that.className.match('SW_fun2') != null || that.className.match('SW_fun') != null)) || that.className == 'expand' || that.getAttribute('node-type') == 'imagesBox';
        }

        if(browserTest){
            //判断是否为多图模式，that.getAttribute('node-type') == 'imagesBox' 为非转发多图的状态。
            multiPics = false;
            ...
            //获取图片参数
            format = that.getElementsByTagName('IMG')[0].src.replace(/.*(\....)$/,'$1');//图片格式
            cdn = that.getElementsByTagName('IMG')[0].src.replace(/^.*?\/\/ww(.).*/,'$1');//图片 CDN 地址
            para = that.children[0].getElementsByClassName('show_big')[0].getAttribute('action-data').replace(/[upm]id=/g,'').split('&');
            pid = para[0];
            mid = para[1];
            uid = para[2];
...
try{
    ...
    if(typeof(that.children) != 'undefined' && that.children.length > 0){
        ...
        if(isFirefox){
            var browserTest  = (that.className.match('WB_media_expand') != null && (that.className.match('SW_fun2') != null || that.className.match('SW_fun') != null) && that.style.display != "none");
        }else{
            var browserTest = (that.className.match('WB_media_expand') != null && (that.className.match('SW_fun2') != null || that.className.match('SW_fun') != null));
        }

        if(browserTest){

            //修正节点
            quote = that.children.length == 1;
            multiPics = false;
            if(typeof(that.children[0].getAttribute('node-type')) == 'string'){
                if(that.children[0].getAttribute('node-type') == 'imagesBox'){
                    multiPics = true;
        ...
        if(browserTest){
            ...
            if(multiPics && that.getAttribute('node-type') == 'imagesBox'){
                that.children[0].addEventListener('mouseover',function(){
                    format = that.getElementsByTagName('IMG')[0].src.replace(/.*(\....)$/,'$1');
                    cdn = that.getElementsByTagName('IMG')[0].src.replace(/^.*?\/\/ww(.).*/,'$1');
                    para = that.children[0].getElementsByClassName('show_big')[0].href;
                    pid = para.replace(/.*\/pid\/(.*)\?.*/,'$1');
                    mid = para.replace(/.*\/mid\/(.*?)\/.*/,'$1');
                    uid = para.replace(/.*weibo.com\/(.*?)\/.*/,'$1');
                    that.children[0].children[8].href = 'http://photo.weibo.com/' + uid + '/wbphotos/large/mid/' + mid + '/pid/' + pid;
...
try{
    ...
    if(typeof(that.children) != 'undefined' && that.children.length > 0){
        ...
        if(isFirefox){
            var browserTest  = (that.getAttribute('node-type') == 'imagesBox' && that.style.display != "none") || (that.className.match('comment') != null && that.className.match('W_textc') != null && that.style.display != "none") || (that.className == 'expand' && (that.parentNode.style.display != "none" && that.style.display != "none")) ;
        }else{
            var browserTest = that.getAttribute('node-type') == 'imagesBox' || (that.className.match('comment') != null && that.className.match('W_textc') != null) || that.className == 'expand';
        }
    ...
    if(typeof(that.children) != 'undefined' && that.children.length > 0){
        ...
        if(browserTest){
            if(that.className == 'MIB_assign' && that.id.match('disp_')){
                ...
                mid = that.parentNode.parentNode.parentNode.id.replace('disp_','');
                uid = that.parentNode.parentNode.parentNode.parentNode.children[3].children[0].children[0].children[0].href.replace(/.*weibo.com\/(.*?)\/.*/,'$1');
            }else if(that.className == 'blogPicOri' && that.children.length > 1){
                that = that.children[1];
                mid = that.parentNode.id.replace('disp_','');
                uid = that.parentNode.parentNode.children[0].getElementsByClassName('source_att')[0].children[0].href.replace(/.*weibo.com\/(.*?)\/.*/,'$1');
                rid = that.parentNode.parentNode.children[0].getElementsByClassName('source_att')[0].children[0].children[1].getAttribute('rid'); //怎么特么又跑出来个 rid
                mid = rid; //用 rid 替换掉 mid，下面就不用在替换了
            }
            format = that.children[1].src.replace(/.*(\....)$/,'$1');
            cdn = that.children[1].src.replace(/^.*?\/\/ww(.).*/,'$1');

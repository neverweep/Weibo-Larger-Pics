...
//判断浏览器以确定读取和储存方法
var _type, _mode, _css, _cdn, GM_getValue;
...
var search = window.location.host === 's.weibo.com', //判断搜索页面
    ...
    photoFluid = !!window.location.href.match(/weibo\.com\/p\/\d+/);
//正则表达式
var reg3 = /.*weibo.com\/(.*?)\/.*/,
    reg4 = /.*uid=(\d*)&?.*/,
    reg5 = /.*mid=(\d*)&?.*/,
...
wlp_floatbar = {
    ...
    property : function(uid, mid, pid, format, cdn){
        ...
        document.onkeydown = function(e){
            e = e || window.event;
            if('65 83 68 87'.indexOf(e.keyCode.toString()) >= 0){
                e.stopPropagation();
                e.preventDefault();
                e.returnValue = false;
            }
            switch(e.keyCode){//A S D W Q
                ...
                case 87 : initGallery();break;
                case 81 : window.open($id('wlp_floatbar_4').href, '_blank ' + Math.random());break;
            }
        }
    },
...
var bindSmall = {
    ...
    photoFluid : function(){
        wlp_bind.photoFluid = addNodeInsertedListener('img.photo_pic[src*="sinaimg"]:hover, img.photo_pict[src*="sinaimg"]:hover', function(e){
            that = e.target || event.target;
            wlp_floatbar.close();
...
ftDiv.innerHTML = '<a href="javascript:;"target="_blank"id="wlp_floatbar_1"title="快捷键(A) 进入相册大图页面">图</a><a href="javascript:;"id="wlp_floatbar_2"title="快捷键(S) 加入该图地址到收集面板">集</a><a href="javascript:;"target="_blank"id="wlp_floatbar_3"title="快捷键(D) 大图原始地址，在此点右键可以另存图像或者复制地址转发给别人">源</a><a href="javascript:;"id="wlp_floatbar_5"title="快捷键(W) 使用画廊模式浏览本页大图">览</a><a href="javascript:;"target="_blank"id="wlp_floatbar_4"title="快捷键(Q) 访问图像原始上传者微博">访</a>';
...
$id('wlp_floatbar_4').onclick = function(){wlp_floatbar.close();}
$id('wlp_floatbar_5').onclick = function(){wlp_floatbar.close();}
...
function initGallery(){
    ...
    document.onkeydown = function(e){
        ...
        switch(e.keyCode){//ESC S 右上左下 Z C X V B
            case 27 : gallery.control.exitGallery();break;
            case 83 : gallery.calcPos(gallery.img.height, gallery.img.width, '');break;
...
addNodeInsertedListener('.ficon_set:hover', function(e){
    if(!document.querySelector("#wlp_setting_btn")){
        ...
        document.querySelector(".gn_topmenulist_set").children[0].insertBefore(li, document.querySelector(".gn_topmenulist_set").children[0].childNodes[10]);
        var li = document.createElement('li');
        li.className = "line S_line1";
        document.querySelector(".gn_topmenulist_set").children[0].insertBefore(li, document.querySelector(".gn_topmenulist_set").children[0].childNodes[10]);
    }
});
